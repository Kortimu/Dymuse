...
@ApplyOptions<CommandOptions>({
    ...
    detailedDescription: "A command that provides the user with additional information about a specific user, such as the creation date, join date and more.",
})
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        if (targetMember.joinedAt !== null) {
            infoEmbed.addFields({
                ...
                name: 'Progress',
                value: `Level: **${level}**\nXP: ${xp}/${getNeededXP(level)} (${Math.round(xp/getNeededXP(level)*100)}%)`,
                inline: true
            })
