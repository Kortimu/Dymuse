    ...
    return send(message, {
      embeds: [
        new MessageEmbed()
          ...
          .setTitle('Error')
          .setDescription(
            'I am not good enough to remove something non-existant.',
          ),
      ],
    }).then((msg) => {
      setTimeout(() => {
    ...
    return send(message, {
      embeds: [
        new MessageEmbed()
          .setColor('#FF0000')
          .setTitle('Error')
          .setDescription('Just because you entered a number, does not mean I can remove it. Stop thinking I am so dumb, human.')
      ]
    })
  }
