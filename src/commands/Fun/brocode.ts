...
@ApplyOptions<CommandOptions>({
  ...
  detailedDescription: 'A command that sends either a specific Bro Code rule, a random one, or all of the rules.',
  preconditions: ['TestOnly'],
  syntax: '[1-26/all]',
  examples: ['brocode 21', 'bcode all', 'thecode'],
  notes: ['If no argument is specified, a random rule will be sent.', 'If the user picks all rules, they get it sent in PMs, to not clutter the channel too much.']
})
...
async function sendRule(option: string, message: Message) {
  ...
  if (parsedOption <= broCode.length && parsedOption >= 0) {
    return send(message, {
      embeds: [
        ruleEmbed
          .setTitle(`Rule #${selectedOption}`)
          .setDescription(`${broCode[parsedOption]}`)
      ]
    })
  }
  if (option === 'all') {
